    ...
    <section id="works">
      <div className='max-w-7xl mx-auto my-20 px-4'>
        ...
        className='text-center mb-12 border-t border-neutral-800'>
          <h2 className='text-3xl lg:text-5xl mt-20 tracking-tighter bg-gradient-to-t from-neutral-50 via-neutral-300 to-neutral-500 bg-clip-text text-transparent'>
          ...
          {HOW_IT_WORKS_CONTENT.steps.map((step, index) => (
            ...
            key={index} className='bg-neutral-900 p-6 rounded-xl shadow-lg flex flex-col justify-between'>
              ...
              <div className='flex justify-center'>
                <img src={step.imageSrc} alt={step.title} className='rounded-lg max-w-full h-auto' />
              </div>
              {step.users && (
                <div className='flex justify-between items-center mt-4'>
                  {step.users.map((user, userIndex) => (
                    <img 
                      ...
                      className='w-10 h-10 rounded-full border-2 border-black' // Example size and styling
                    />
                  ))}
                </div>
              )}
